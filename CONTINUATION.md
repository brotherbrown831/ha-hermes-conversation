...
- `find_home_entities` v3 (same day): entity ALIASES + device-inherited AREAS now searchable. HA's REST registry endpoints 404 in 2026.8.3 and `config/entity_registry/list` returns partial records without aliases, so the adapter fetches the full per-entity record (`config/entity_registry/get`, sequential send/recv — concurrent WS I/O corrupts matching) plus `config/device_registry/list` for device→area, cached 5 min and warmed at adapter startup. Alias substring matches get a +50 rank boost; entity-level area is applied, then the device's. Verified: "garage door" → `cover.ratgdo32_e15de0_door` (area Garage, open) found ONLY via alias ["garage door", ...]; ~79 entities carry aliases in this instance; "Yes, the garage door is open." answered correctly through HA chat.
- TOOLS ARE WARM (same day): `platform_toolsets.api_server` entries are TOOLSET names only — `server:tool` entries are invalid in this Hermes version (issue #38798 warning) and leave the platform with 0 visible tools; MCP tools are always deferred behind the tool_search bridge (July-2026 semantics: `should_activate` returns True for ANY MCP tool). Fix: set `tools.tool_search.enabled: off` in the VV profile config — pass-through mode injects all 8 tool schemas into every request. Measured net prompt delta only +~180 tokens (the bridge listing + 2 bridge schemas were already in the prompt; they get swapped for the 8 full schemas). Result: "Is the garage door open?" went 3→2 API calls, 2→1 tool turns, ~6.2s→~4.2s, zero tool_search/tool_describe calls.
